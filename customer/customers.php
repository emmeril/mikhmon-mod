...
$customerMitraNames = array();
foreach (mikhmonGetUsers('mitra', $session) as $mitra) {
  if (isset($mitra['id'])) $customerMitraNames[(string) $mitra['id']] = (string) ($mitra['name'] ?? $mitra['username'] ?? '');
}
$customerMitraFilterOptions = array();
foreach ($customers as $customer) {
  $mitraId = (string) ($customer['mitra_id'] ?? '');
  $mitraName = $mitraId !== '' && isset($customerMitraNames[$mitraId]) ? $customerMitraNames[$mitraId] : 'Belum ditetapkan';
  $customerMitraFilterOptions[$mitraName === 'Belum ditetapkan' ? '' : $mitraId] = $mitraName;
}
asort($customerMitraFilterOptions, SORT_NATURAL | SORT_FLAG_CASE);
...
if (!empty($routerConnected)) {
  ...
  foreach (array('hotspot' => array('/ip/hotspot/active/print', 'user'), 'pppoe' => array('/ppp/active/print', 'name')) as $serviceType => $activeDefinition) {
    $activeRows = $API->comm($activeDefinition[0]);
    if (is_array($activeRows) && customerListApiError($activeRows) === '') foreach ($activeRows as $activeRow) {
      $activeUsername = trim((string) ($activeRow[$activeDefinition[1]] ?? ''));
      if ($activeUsername !== '') $customerActiveUsers[$serviceType][$activeUsername] = true;
    }
  }
}
?>
<div class="row"><div class="col-12"><div class="card">
  ...
  <div class="card-body">
    ...
    <style>
      ...
      @media (prefers-reduced-motion: reduce) { #dataTable .customer-online-indicator { animation:none; } }
      ...
      <?php foreach ($customers as $customerIndex => $customerRow): ?>
        <?php
          ...
          foreach ($customerServices as $customerService) {
            $serviceType = ($customerService['service'] ?? '') === 'pppoe' ? 'pppoe' : 'hotspot';
            $serviceUsername = trim((string) ($customerService['username'] ?? ''));
            if ($serviceUsername !== '' && isset($customerActiveUsers[$serviceType][$serviceUsername])) { $customerIsOnline = true; break; }
          }
...
$(function() {
  ...
  function filterCustomers() {
    ...
    var mitraId = $('#customerMitraFilter').val();
    var visible = 0;
    $('.customer-row').each(function() {
      ...
      var matchesMitra = mitraId === 'all' || String(row.data('mitra-id')) === mitraId;
      ...
      var show = matchesSearch && matchesService && matchesStatus && matchesMitra;
      row.toggle(show);
      if (show) visible++;
    });
