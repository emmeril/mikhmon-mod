...
roleTestAssert(mikhmonSaveCustomerIdentity('router-a', $identityOnly, 'Identitas Baru Update', '08120000000', 'Alamat Update', $mitraId) === $identityOnly, 'identity can be updated independently');
$updatedIdentity = mikhmonFindCustomer('router-a', $identityOnly);
roleTestAssert(count(mikhmonCustomerServices($updatedIdentity)) === 1 && ($updatedIdentity['due_date'] ?? '') === '2026-10-10', 'identity update preserves services and due date');
roleTestAssert(mikhmonSaveCustomerIdentity('router-a', $customerTwo, 'Pelanggan A', '', '', '') === false, 'identity names cannot be duplicated during edit');
...
roleTestAssert(mikhmonAssignedCustomerCount($mitraId) === 2, 'assigned customer identity count is tracked');
...
roleTestAssert(mikhmonCanOpenMainRoute('customer-identity-add'), 'mitra can add customer identities');
roleTestAssert(mikhmonCanOpenMainRoute('customer-identity-list'), 'mitra can manage customer identities');
