...
<?php endif; ?>
<script type="text/javascript">
// Keep session-name validation while allowing a custom sidebar brand.
var sessionName = document.getElementById("sessname");
if (sessionName) {
  function checkSessionName() {
    var value = sessionName.value;
    if (value === "mikhmon" || value === "MIKHMON" || value === "Mikhmon") {
      alert("You cannot use " + value + " as a session name.");
      sessionName.value = "";
      window.location.reload();
    }
  }
  sessionName.onkeyup = checkSessionName;
  sessionName.onchange = checkSessionName;
}
var pingButton = document.getElementById("ping_test");
if (pingButton && sessionName) {
  pingButton.onclick = function () {
    $("#ping").load("./status/ping-test.php?ping&session=" + encodeURIComponent(sessionName.value));
  };
}
</script>
