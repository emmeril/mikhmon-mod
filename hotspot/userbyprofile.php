  ...
  <div class="card-body">
    <div class="overflow box-bordered" style="max-height:75vh"><table id="voucherProfileSummary" class="table table-bordered table-hover text-nowrap">
      ...
      <?php $index = 0; foreach ($stats as $profileName => $row): $index++; $profile = $row['profile']; $profileMode = mikhmonBillingProfileExpiredMode('hotspot', $profile); ?>
        <tr><td class="text-center"><?= $index; ?></td><td><?= htmlspecialchars($profileName, ENT_QUOTES); ?></td><td><?= htmlspecialchars(mikhmonVoucherProfileModeLabel($row['mode']), ENT_QUOTES); ?></td><td class="text-center"><?= $row['total']; ?></td><td class="text-center text-success"><strong><?= $row['unused']; ?></strong></td><td class="text-center"><?= $row['used']; ?></td><td class="text-center text-danger"><?= $row['expired_known'] ? $row['expired'] : '-'; ?></td><td class="text-nowrap"><?php if ($profileMode !== 'none'): ?><a class="btn bg-info" href="./?hotspot=users&amp;profile=<?= rawurlencode($profileName); ?>&amp;session=<?= rawurlencode($session); ?>" title="Buka daftar voucher profile <?= htmlspecialchars($profileName, ENT_QUOTES); ?>"><i class="fa fa-folder-open"></i> Buka</a> <a class="btn bg-primary" href="./?hotspot-user=generate&amp;genprof=<?= rawurlencode($profileName); ?>&amp;session=<?= rawurlencode($session); ?>"><i class="fa fa-ticket"></i> Generate</a><?php endif; ?></td></tr>
      <?php endforeach; ?>
